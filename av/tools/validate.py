...
from matching import record_index, validate_document, validate_need, validate_offer
...
def migration_checks(checks):
    old = read(relative_file("av/support/history/v0.1-proposed/vocabulary/terms.json"))
    migration = read(relative_file("av/support/migration-v0.2.json"))
    terms = model.TERMS
...
def inventory_and_json(checks, *, core_only=False):
    ...
    ontology = Graph().parse(relative_file("av/ontology.ttl"), format="turtle")
    shapes = Graph().parse(relative_file("av/av.shacl.ttl"), format="turtle")
    ...
    context = read(relative_file("av/context.jsonld"))
    checks.check("Context mappings are generated exactly",
                 relative_file("av/context.jsonld").read_bytes() == json_bytes(model.make_context()))
    ...
    samples = read(relative_file("av/tools/fixtures/core.json"))
    ...
    seen = set()
    records = record_index(documents.values())
    for name, document in documents.items():
        validate_av_keys(document)
        graph = graph_for(document)
        audit_av_graph(graph)
        validate_document(document, records=records)
        original = {AV[key[3:]] for node in nodes(document) for key in node if key.startswith("av:")}
        assertions = assert_projection(document, graph)
        checks.check("Complete AV subject/value/datatype/cardinality assertions survive expansion: " + name,
                     True, {"assertions": assertions})
        seen |= original
...
def native_dependencies(checks):
    manifest = read(relative_file("av/support/upstream/wot/manifest.json"))
    for item in manifest["files"]:
        actual = metadata("av/support/upstream/wot/" + item["path"])
    ...
    for name, expected in (("td-context.jsonld", TD_CONTEXT_SHA), ("td-schema.json", SCHEMA_SHA)):
        checks.check("Fixed upstream artifact hash: " + name,
                     sha(relative_file("av/support/upstream/wot/" + name).read_bytes()) == expected)
    ...
    context = read(relative_file("av/support/upstream/wot/td-context.jsonld"))
...
def run_unit_tests(checks):
    suite = unittest.defaultTestLoader.discover(str(ROOT / "av" / "tools" / "tests"), pattern="test_*.py")
    output = io.StringIO()
...
def main():
    ...
    if not args.core_only:
        manifest = read(relative_file("av/support/publication/release-manifest.json"))
        for item in manifest["files"]:
        ...
        checks.check("Release manifest excludes its own digest",
                     all(item["path"] != "av/support/publication/release-manifest.json" for item in manifest["files"]))
    for name in sorted(managed):
        if name.startswith(("av/support/history/", "av/support/upstream/")) or not relative_file(name).is_file():
            continue
    ...
    result = {
        ...
        "limitations": [
            ...
            "RFC 6570 scalar expansion requires explicit invocation parameters; no native protocol or credentials are exercised.",

...
class CoreFixture(unittest.TestCase):
    def setUp(self):
        self.f = copy.deepcopy(read(relative_file("av/tools/fixtures/core.json")))
...
class NativeFormTests(CoreFixture):
    ...
    def test_combo_security_requires_resolved_acyclic_members(self):
        ...
        source["securityDefinitions"] = {
            "combined": {"scheme": "combo", "allOf": ["missing", "known"]},
            "known": {"scheme": "basic"},
        }
        with self.assertRaisesRegex(ValueError, "Unresolved"):
            resolve_form(self.f["formReference"], self.documents())
        source["securityDefinitions"]["combined"]["allOf"] = ["combined", "known"]
...
class DocumentationTests(CoreFixture):
    def test_literal_excerpt_pointers_reject_stale_content(self):
        ...
        text = "<!-- example: av/tools/fixtures/core.json#/rational -->\n```json\n" + value + "```\n"

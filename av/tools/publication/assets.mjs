...
import { REPO_ROOT, SUPPORT_ROOT, readJSON, sha256 } from "./lib.mjs";

export async function verifiedAssets({ root = REPO_ROOT } = {}) {
    const support = path.join(root, "av", "support", "publication");
    const policy = await readJSON(path.join(support, "publication-policy.json"));
    const lock = await readJSON(path.join(support, "assets.lock.json"));
    if (process.versions.node !== policy.node.version || lock.node !== policy.node.version) throw new Error(`Reviewed Node ${policy.node.version} is required`);
    const pkg = await readJSON(path.join(root, "av", "tools", "publication", "package.json"));
    ...
    const notices = await readFile(path.join(support, "THIRD-PARTY-NOTICES.txt"));
    ...
    for (const item of lock.assets) {
        ...
        const body = await readFile(path.join(support, ...item.path.split("/")));

...
def validate_example(value):
    ...
    fixtures = read(relative_file("av/tools/fixtures/core.json"))
...
def snippet_references(text, source_path=None):
    aliases = {}
    if source_path is not None:
        source_name = source_path.relative_to(ROOT).as_posix()
        aliases = read(relative_file("av/support/publication/specification.json"))["sourceAliases"].get(source_name, {})
    markers = list(re.finditer(r"<!--\s*example:\s*([^\s#]+)#([^\s]*?)\s*-->", text))
    for marker in markers:
        name, selector = marker.group(1), marker.group(2)
        if source_path is None or name in aliases:
            target = relative_file(aliases.get(name, name))
        else:
            target = source_path.parent.joinpath(*name.replace("\\", "/").split("/")).resolve()
            if not target.is_relative_to(ROOT) or target.is_symlink():
                raise ValueError("Escaped/symlinked documentation excerpt: " + name)
        sample = pointer(read(target), selector)
...
def run(checks, *, core_only=False):
    specification = relative_file("av/spec.md").read_text(encoding="utf-8")
    assemble(specification, model.TERMS, check=True)
    from schema_contract import SCHEMA_KEYWORDS, SCHEMA_MAPS, SCHEMA_ARRAYS, SCHEMA_SINGLE
    grammar = model.TERMS["processing_contract"]
    for field, expected in (("keywords", SCHEMA_KEYWORDS), ("schema_map_positions", SCHEMA_MAPS),
                            ("schema_array_positions", SCHEMA_ARRAYS), ("schema_single_positions", SCHEMA_SINGLE)):
        if set(grammar[field]) != expected or len(grammar[field]) != len(expected):
            raise AVError("AV_EDITION_INCONSISTENT", "Normative inventory grammar and executable contract disagree",
                          stage="documentation", location="/processing_contract/" + field)
    checks.check("Root specification contains all 100 inventory-owned entries and 101 paired regions", True,
                 {"entries": len(list(sources(model.TERMS))), "regions": 101, "schemaKeywords": len(SCHEMA_KEYWORDS)})
    rendered = relative_file("av/support/reference/terms.md").read_text(encoding="utf-8")
    ...
    paths = {name for name in paths if not name.startswith(("av/support/upstream/", "av/support/history/", "av/support/research/"))}
    if core_only:
    ...
    for name in sorted(paths):
        ...
        if path.suffix in {".json", ".jsonld"}:
            ...
            snippet_count += snippet_references(text, source_path=path)

...
Authority is `onvif\samples\reference-runtime\native-media\build-p0\final-cohort.json`, with the fresh
...
[onvif-media.md](../notes/onvif-media.md). The final local handoff/report are
...
`onvif\samples\reference-runtime\native-media\dependencies.lock.json` records the exact distribution,
...
See `onvif\samples\reference-runtime\native-media\patches\README.md` for private vendoring and upstream

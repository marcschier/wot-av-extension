...
from native_paths import NATIVE_ROOT, TOOL_ROOT
...
def main():
    ...
    allowed = (Path(os.environ["TEMP"]) / "acf-media-research").resolve()
    cache = (TOOL_ROOT / ".cache").resolve()
    legacy_task = root.parent == allowed and root.name.startswith("onvif-native-p0-")
    cache_task = root.parent == cache and root.name.startswith("qualification-")
    if not legacy_task and not cache_task:
        parser.error("Use a named onvif-native-p0-* task-local TEMP directory or a qualification-* child of the native tool .cache")
    if not (root / ".onvif-native-p0").is_file():
        parser.error("The task ownership marker is missing")
    lock = json.loads((NATIVE_ROOT / "sources.lock.json").read_text())

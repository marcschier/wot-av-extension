...
$cacheParent = [IO.Path]::GetFullPath((Join-Path $NativeToolRoot '.cache'))
$TaskRoot = [IO.Path]::GetFullPath($TaskRoot)
$legacyTask = $TaskRoot.StartsWith("$allowedParent\", [StringComparison]::OrdinalIgnoreCase) -and
    [IO.Path]::GetFileName($TaskRoot) -like 'onvif-native-p0-*'
$cacheTask = [IO.Path]::GetDirectoryName($TaskRoot).Equals($cacheParent, [StringComparison]::OrdinalIgnoreCase) -and
    [IO.Path]::GetFileName($TaskRoot) -like 'qualification-*'
if (-not $legacyTask -and -not $cacheTask) {
    throw "TaskRoot must be a named onvif-native-p0-* task under $allowedParent or a qualification-* child under $cacheParent."
}
$manifest = Get-Content (Join-Path $NativeProjectRoot 'dependencies.lock.json') -Raw | ConvertFrom-Json

...
def main():
    ...
    try:
        ...
        with tarfile.open(tarball, "r:gz") as archive:
            ...
            canonical = json.load(archive.extractfile("package/dist/catalog-data/generated/manifest.json"))
            models = json.load(archive.extractfile("package/dist/catalog-data/models.json"))["models"]
            ...
            for source in (ROOT / "onvif" / "samples" / "reference-runtime" / "src").rglob("*.ts"):
                relative = source.relative_to(ROOT / "onvif" / "samples" / "reference-runtime" / "src").as_posix()[:-3]
            ...
            assert provenance["canonicalArtifacts"] == len(canonical["artifacts"]) + 1
            assert provenance["modelDocuments"] == len(models)
            assert {item["path"] for item in flat["artifacts"]} == set(canonical["packageAssets"])
            source_lock = json.load(archive.extractfile("package/dist/source-documents/sources.lock.json"))
            assert provenance["sourceDocuments"] == sum(source["storage"] == "vendored" for source in source_lock["sources"]) == 43
        ...
        workspace_prefix = "onvif/samples/reference-runtime/"
        ...
        shutil.copyfile(ROOT / "onvif" / "tools" / "onvif-installed-smoke.cjs", install / "installed-smoke.cjs")
...
import { loadPackagedProjectionCatalog, packagedArtifactPath, deriveAdapterTd,
    assertSemanticAdapterValue, type SemanticAdapterTdOptions, type SemanticAdapterProjection } from "@wot-av/binding-onvif/catalog";
...
const semantic: (catalog: ReturnType<typeof loadPackagedProjectionCatalog>["registry"],
    options: SemanticAdapterTdOptions) => SemanticAdapterProjection = deriveAdapterTd;
void [runtime, directory, policy, options, loadPackagedProjectionCatalog, packagedArtifactPath,
    project, MediaExecutor, decodeMetadata, createDiscoveryEngine, NativeEventClient, semantic, assertSemanticAdapterValue];
"""
        ...
        bundle = json.loads((install / "cli-models" / "onvif-bundle-manifest.json").read_bytes())
        expected_exports = {model["id"].removeprefix("https://example.org/wot/onvif/") for model in models}
        expected_exports.update({
            "context/v0.1", "context/publication/v1", "schemas/payloads.schema.json",
            "schemas/form.schema.json", "schemas/snapshot.schema.json", "schemas/requirements.schema.json"
        })
        assert set(bundle["files"]) == expected_exports, "CLI model/schema/context library differs from the installed canonical library"
        assert {path.relative_to(install / "cli-models").as_posix() for path in exported_files} == expected_exports | {"onvif-bundle-manifest.json"}
        for name, expected_hash in bundle["files"].items():
            assert hashlib.sha256((install / "cli-models" / name).read_bytes()).hexdigest() == expected_hash

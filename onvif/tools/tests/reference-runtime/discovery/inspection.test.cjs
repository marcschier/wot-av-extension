...
test("every allowed read resolves to its exact binding, portType and source operation in the locked WSDL", () => {
    ...
    const lock = JSON.parse(fs.readFileSync(path.join(root, "onvif", "sources.lock.json"), "utf8"));
    const cache = new Map();
    ...
    for (const contract of h.READ_CONTRACTS) {
        ...
        if (!cache.has(source.id)) cache.set(source.id, h.wire(fs.readFileSync(path.join(root, "onvif", "support", "upstream", "onvif", ...source.relativePath.split("/")))));

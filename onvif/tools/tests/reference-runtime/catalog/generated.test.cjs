...
const { PACKAGE_ASSETS } = require("../../../../samples/reference-runtime/dist/catalog/artifacts");
const { operationSchemaKey, ONVIF_CONTEXT } = require("../../../../samples/reference-runtime/dist/catalog/schemas");
...
test("generated: --check is deterministic and performs no HTTP, DNS or socket retrieval", () => {
    const tool = join(root, "onvif", "tools", "generate-onvif.cjs");
    const script = `
...
test("generated: manifest bytes, four-space JSON, codec privacy and additive ONVIF context agree", () => {
    const manifest = JSON.parse(readFileSync(join(base, "model-manifest.json"), "utf8"));
    ...
    for (const entry of manifest.artifacts) {
        const physical = require("../../../../samples/reference-runtime/dist/catalog/artifacts").physicalArtifactPath(root, entry.path);
    ...
    assert.equal(catalog.requirements.diagnostics.filter((entry) => entry.severity === "error").length, 0);
    assert.equal(catalog.requirements.diagnostics.filter((entry) => entry.code === "native-topic-element-root-needs-description").length, 2);
});
...
test("generated: artifact paths work outside the checkout and corrupted artifacts fail closed", () => {
    ...
    try {
        const source = resolve(dirname(require.resolve("../../../../samples/reference-runtime/dist/catalog/packaged")), "..");
...
test("generated: npm's package selection includes every required registry/schema/context asset", () => {
    ...
    const result = command(process.execPath, [npm, "pack", "--dry-run", "--ignore-scripts", "--json"],
        { cwd: join(root, "onvif", "samples", "reference-runtime") });

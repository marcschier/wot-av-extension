...
const { generateModels, profileModelId } = require("../../../../samples/reference-runtime/dist/catalog/models");
const { modelIndex, assertModelImports, resolveModel } = require("../../../../samples/reference-runtime/dist/catalog/model-resolver");
...
const native = require("../../../fixtures/reference-runtime/catalog/native-responses.json").cases;
const goldens = require("../../../fixtures/reference-runtime/catalog/projection-goldens.json");
...
test("projection: draft G interface/behavior interpretation is deterministic without inventing named approval", () => {
    ...
    for (const id of ["G-9.2.3-device-recording-set-track-configuration", "G-9.2.1-device-track-description-update"]) {
        ...
        assert.ok(row);
        assert.equal(row.applicability, "true");
        assert.deepEqual(row.unresolvedEditorialDecisionIds, []);
        assert.ok(row.interpretation.decisionIds.includes("ONVIF-D023"));
        assert.equal(row.interpretation.approval, "pending-named-review");
        assert.equal(row.interpretation.releaseGate, "named-editor-and-independent-review-before-formal-release");
    }
    input.facts["device.feature.dynamicTracks"] = known(false, "explicit-negative-feature");
    const inactive = api.project(input, catalog, policy);
    assert.equal(inactive.assessments.find((row) => row.requirementId === "G-9.2.3-device-recording-set-track-configuration").applicability, "true");
    assert.equal(inactive.assessments.find((row) => row.requirementId === "G-9.2.1-device-track-description-update").applicability, "false");
...
test("projection: deterministic composition has no caller aliases, duplicate client IDs, fake endpoints or missing imports", () => {
    ...
    const nativeType = second.tds.find((entry) => entry.id === goldens.deviceId).links.find((entry) => entry.rel === "type").href;
    const anyComposite = second.models.find((entry) => entry.id === nativeType);

...
const f = require("../../../fixtures/reference-runtime/events/native-fixture.cjs");
...
test("generated event artifacts: independent offline JSON Schema and JSON-LD validate complete Event data and reject malformed Forms", () => {
    ...
    try {
        const model = JSON.parse(readFileSync(join(f.root, "onvif", "models", "events", "PullPointSubscription.tm.json"), "utf8"));
        ...
        const result = spawnSync("python", ["-B", join(f.root, "onvif", "tools", "tests", "reference-runtime", "catalog", "validate-generated.py"), f.root, input],

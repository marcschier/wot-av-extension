...
from gate import FIXTURE_ROOT, NATIVE_ROOT, TEST_ROOT, TOOL_ROOT, binary_inventory
...
# Frozen logical membership from relocation-plan.json nativeCohort.sourceMap.
SOURCE_PATHS = {
    "CMakeLists.txt": NATIVE_ROOT / "CMakeLists.txt",
    "cmake/config.h.in": NATIVE_ROOT / "cmake" / "config.h.in",
    "cmake/rtsp-enumtypes.c.in": NATIVE_ROOT / "cmake" / "rtsp-enumtypes.c.in",
    "dependencies.lock.json": NATIVE_ROOT / "dependencies.lock.json",
    "patches/classic-media-v2.patch": NATIVE_ROOT / "patches" / "classic-media-v2.patch",
    "patches/classic-rtsp-p0.patch": NATIVE_ROOT / "patches" / "classic-rtsp-p0.patch",
    "patches/p0_digest.c": NATIVE_ROOT / "patches" / "p0_digest.c",
    "patches/p0_digest.h": NATIVE_ROOT / "patches" / "p0_digest.h",
    "sources.lock.json": NATIVE_ROOT / "sources.lock.json",
    "src/ipc.c": NATIVE_ROOT / "src" / "ipc.c",
    "src/ipc.h": NATIVE_ROOT / "src" / "ipc.h",
    "src/jpeg.c": NATIVE_ROOT / "src" / "jpeg.c",
    "src/jpeg.h": NATIVE_ROOT / "src" / "jpeg.h",
    "src/metadata.c": NATIVE_ROOT / "src" / "metadata.c",
    "src/metadata.h": NATIVE_ROOT / "src" / "metadata.h",
    "src/options.c": NATIVE_ROOT / "src" / "options.c",
    "src/options.h": NATIVE_ROOT / "src" / "options.h",
    "src/terminal.c": NATIVE_ROOT / "src" / "terminal.c",
    "src/terminal.h": NATIVE_ROOT / "src" / "terminal.h",
    "src/worker.c": NATIVE_ROOT / "src" / "worker.c",
    "test/codec_fixture.py": FIXTURE_ROOT / "codec_fixture.py",
    "test/codec_gate.py": TEST_ROOT / "codec_gate.py",
    "test/cohort_gate.py": TEST_ROOT / "cohort_gate.py",
    "test/gate.py": TEST_ROOT / "gate.py",
    "test/make-test-certificate.cjs": TEST_ROOT / "make-test-certificate.cjs",
    "test/node-ipc-smoke.cjs": TEST_ROOT / "node-ipc-smoke.cjs",
    "test/p0_ipc.c": FIXTURE_ROOT / "p0_ipc.c",
    "test/p0_ipc.h": FIXTURE_ROOT / "p0_ipc.h",
    "test/p0_metadata.c": FIXTURE_ROOT / "p0_metadata.c",
    "test/p0_metadata.h": FIXTURE_ROOT / "p0_metadata.h",
    "test/p0_worker.c": FIXTURE_ROOT / "p0_worker.c",
    "test/rtsp_fixture.py": FIXTURE_ROOT / "rtsp_fixture.py",
    "test/runtime_fixture.py": FIXTURE_ROOT / "runtime_fixture.py",
    "test/runtime_gate.py": TEST_ROOT / "runtime_gate.py",
    "test/runtime_ipc.py": TEST_ROOT / "runtime_ipc.py",
    "test/test_digest.c": TEST_ROOT / "test_digest.c",
    "test/test_jpeg.c": TEST_ROOT / "test_jpeg.c",
    "test/test_metadata.c": TEST_ROOT / "test_metadata.c",
    "test/test_metadata_v2.c": TEST_ROOT / "test_metadata_v2.c",
    "tools/build.ps1": TOOL_ROOT / "build.ps1",
    "tools/fetch-sdk.ps1": TOOL_ROOT / "fetch-sdk.ps1",
    "tools/fetch-sources.py": TOOL_ROOT / "fetch-sources.py",
    "tools/prepare-source.py": TOOL_ROOT / "prepare-source.py",
}
...
def source_inventory():
    # Path retains originalWindowsFingerprintKey on Windows and POSIX keys elsewhere.
    return {str(Path(*logical.split("/"))): sha256(path) for logical, path in SOURCE_PATHS.items()}

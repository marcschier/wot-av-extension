...
if __package__:
    ...
    from repository import REPO_ROOT as ROOT
DEFAULT_LOCK = ROOT / "onvif" / "sources.lock.json"
DEFAULT_PROFILES = ROOT / "onvif" / "profile-editions.json"
HOSTS = frozenset((
...
def _read_source(source, root, cache):
    ...
    if source["storage"] == "vendored":
        candidates.append(_safe_path(root, "onvif/support/upstream/onvif/" + source["relativePath"]))

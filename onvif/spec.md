...
The current inventory edition is **0.2-proposed**: it retains the 68 original term contracts plus `Semantic` and the opaque JSON `projection` record. The 17 September 2026 draft revision deliberately renames 18 class IRIs, removing their `Thing` suffix and naming the native-contract classification `NativeContract`. This is a breaking proposal-vocabulary change, not an additive IRI-compatibility claim. The [informative migration record](support/editorial/standards-decisions.json#/vocabularyRevision) gives the complete mapping. Regenerate dependent TDs, TMs and vocabulary artifacts; no obsolete class aliases are declared. A publisher MUST serve the current inventory revision or a verified local equivalent; the unchanged unhosted context identifier does not establish network publication.
...
The matrices below give the editorial contract for all 68 original entries under their current names. The sole term inventory records these definitions, ownership, cardinality, omission behavior, examples and references. The generated reference also declares `Semantic` and `projection` as the two additional terms for model edition 0.2-proposed. Generic comments such as "project metadata" are not sufficient definitions.

ONVIF vocabulary class local names MUST NOT end in `Thing`. Class names such as `NativeContract`, `Device`, `Resource` and `Semantic` retain their existing meanings and host domains. Standard `td:Thing`, `tm:ThingModel`, generic WoT API names, native XML QNames, and existing model document IDs and filenames are separate identities and are not renamed.

Vocabulary examples show the annotation in its actual TD location: root class membership, a Property/DataSchema, an Action or its Form, an Event Form, or a security definition. Terms whose host is a reusable model are explicitly shown as Thing Model excerpts, not as concrete TDs. The inventory's authoring-only `exampleHost` selects that placement; it is not an RDF term. An adjacent `td-excerpt` directive selects retained paths from the linked complete example. A `jsonc` fence uses only standalone `// ...` comments for omitted members; it is illustrative, not a complete JSON document. Ordinary JSON examples, native payloads and normative descriptor grammar remain complete in their declared format.
...
<a id="native-contract-naming"></a>
`NativeContract` is a mapping-defined classification of a genuine native ONVIF contract, not an ONVIF source type name or a device identity. It applies to the reusable native base, operation library and PullPoint Event model; the distinct `Device` and `Resource` classes continue to identify the evidenced device or scoped resource. [ONVIF Core][onvif-core], chapter 5, distinguishes the device as service provider from the client as service requester; this does not make every native contract a device. This rename introduces no subclass or equivalence assertion, and class membership never authorizes an EPR or Form target.

| Class | Host and authority; consumer obligation | Absence, errors, and example boundary |
| --- | --- | --- |
| `NativeContract` | Native TM/TD; model author declares a genuine native endpoint contract. Consumer applies native overlay and source rules. | Does not create a server. An adapted USB camera MUST NOT use this class to claim native service support. |
| `Device` | Native device TM/TD; publisher identifies one evidenced device, independent of its IP addresses. | No device is established from a URL alone. Example: one device visible on two NICs remains one device. |
| `Resource` | Native resource TM/TD; publisher retains device, service, kind, parents and token. | Generic kind does not mean absent resource semantics. Unknown kinds retain evidence and a diagnostic rather than an invented subtype. |
...
| `MediaProfile` | Native resource; publisher describes a token-addressed bundle of media configurations. | `fixed` means non-deletable, not immutable. Example: token `main-01` is not a Profile T claim. |
| `Recording` | Native recording resource; publisher preserves recording token and endpoint/service scope. | No implicit creation, search or replay. Two recording resources can have different tracks with the same track-token spelling. |
| `RecordingTrack` | Native resource with a Recording parent; consumer MUST retain that parent in identity and inputs. | Missing parent is invalid for a complete identity. `track1` below two recordings is not one track. |
| `RecordingJob` | Native recording-job resource; publisher describes actual job identity and observed configuration/state. | Requested Active mode is not evidence of transfer. Creation output's adjusted configuration is authoritative. |
| `AccessPoint` | Native access-point resource; publisher retains directional areas, entity references and point-scoped capabilities. | No grant or default enabled state. Two access points can refer to one door. |
| `Door` | Native door resource; publisher describes door identity and separate mode/sensor facts. | Unknown sensed position is not unlocked. This class never authorizes LockDoor or AccessDoor. |
| `AnalyticsModule` | Native module under its analytics-configuration parent; module Name and Type remain distinct. | A QName type or sample frame is not live inference. Missing parent MUST NOT be guessed. |
| `AnalyticsRule` | Native rule under its analytics-configuration parent; preserve rule name, rule-type QName and parameters. | No same-named global XSD type is inferred. `LineCounting` denotes a rule type when used as such. |
| `Receiver` | Native Receiver resource; publisher preserves token, configuration, mode and observed connection state. | A configured URI does not establish reception. No implicit CreateReceiver or connection is permitted. |
| `VideoSource` | Native source resource; publisher preserves source identity separately from encoder/profile settings. | Source options do not imply a supported combination or sensor maximum. |
| `AudioSource` | Native source resource; publisher preserves token and actual source facts. | Does not imply audio in every media profile, an output decoder, or backchannel. |
| `DigitalInput` | Native I/O resource; publisher scopes facts to the input token. | Missing state is unknown. A Boolean `false` is a known value only with valid evidence. |
| `RelayOutput` | Native I/O resource; publisher describes token and supported native configuration/actuation contract. | No actuation default. A successful command is not proof of external electrical or mechanical effect. |
| `SerialPort` | Native serial resource; publisher preserves port identity and supported configuration. | No implicit byte transfer, guessed baud rate, or access to an OS serial device. |
...
| --- | --- |
| IRI | `https://example.org/wot/onvif#NativeContract` |
...
| Requiredness | Use this class only when the native ONVIF contract actually applies to the declared host. Does not create a server. An adapted USB camera MUST NOT use this class to claim native service support. |
...
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-NativeContract](#term-NativeContract); [#native-contract-naming](#native-contract-naming) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/NativeContract).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/NativeContract","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
    ...
    "@type": "onvif:NativeContract"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/0/counterexample).
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#Device` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the Device contract actually applies to the declared host. No device is established from a URL alone. Example: one device visible on two NICs remains one device. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-Device](#term-Device) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/Device).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/Device","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/1/counterexample).

<a id="term-Resource"></a>
#### `onvif:Resource`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#Resource` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the Resource contract actually applies to the declared host. Generic kind does not mean absent resource semantics. Unknown kinds retain evidence and a diagnostic rather than an invented subtype. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-Resource](#term-Resource) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/Resource).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/Resource","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/2/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/CanonicalXmlValue).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/CanonicalXmlValue","retain":["/@context","/properties/value/@type","/properties/value/type","/properties/value/onvif:sourceDataSchema","/properties/value/onvif:validation"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/3/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/XMLQName).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/XMLQName","retain":["/@context","/properties/value/@type","/properties/value/type","/properties/value/properties","/properties/value/required"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/4/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/UsernameTokenSecurityScheme).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/UsernameTokenSecurityScheme","retain":["/@context","/security","/securityDefinitions/example/@type","/securityDefinitions/example/scheme"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/5/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/MutualTlsSecurityScheme).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/MutualTlsSecurityScheme","retain":["/@context","/security","/securityDefinitions/example/@type","/securityDefinitions/example/scheme"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/6/counterexample).

<a id="term-MediaProfile"></a>
#### `onvif:MediaProfile`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#MediaProfile` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the MediaProfile contract actually applies to the declared host. `fixed` means non-deletable, not immutable. Example: token `main-01` is not a Profile T claim. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-MediaProfile](#term-MediaProfile) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/MediaProfile).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/MediaProfile","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:MediaProfile"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/7/counterexample).

<a id="term-Recording"></a>
#### `onvif:Recording`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#Recording` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the Recording contract actually applies to the declared host. No implicit creation, search or replay. Two recording resources can have different tracks with the same track-token spelling. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-Recording](#term-Recording) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/Recording).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/Recording","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:Recording"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/8/counterexample).

<a id="term-RecordingTrack"></a>
#### `onvif:RecordingTrack`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#RecordingTrack` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the RecordingTrack contract actually applies to the declared host. Missing parent is invalid for a complete identity. `track1` below two recordings is not one track. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-RecordingTrack](#term-RecordingTrack) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/RecordingTrack).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/RecordingTrack","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:RecordingTrack"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/9/counterexample).

<a id="term-RecordingJob"></a>
#### `onvif:RecordingJob`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#RecordingJob` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the RecordingJob contract actually applies to the declared host. Requested Active mode is not evidence of transfer. Creation output's adjusted configuration is authoritative. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-RecordingJob](#term-RecordingJob) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/RecordingJob).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/RecordingJob","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:RecordingJob"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/10/counterexample).

<a id="term-AccessPoint"></a>
#### `onvif:AccessPoint`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#AccessPoint` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the AccessPoint contract actually applies to the declared host. No grant or default enabled state. Two access points can refer to one door. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-AccessPoint](#term-AccessPoint) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/AccessPoint).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/AccessPoint","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:AccessPoint"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/11/counterexample).
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#Door` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the Door contract actually applies to the declared host. Unknown sensed position is not unlocked. This class never authorizes LockDoor or AccessDoor. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-Door](#term-Door) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/Door).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/Door","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/12/counterexample).

<a id="term-AnalyticsModule"></a>
#### `onvif:AnalyticsModule`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#AnalyticsModule` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the AnalyticsModule contract actually applies to the declared host. A QName type or sample frame is not live inference. Missing parent MUST NOT be guessed. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-AnalyticsModule](#term-AnalyticsModule) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/AnalyticsModule).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/AnalyticsModule","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:AnalyticsModule"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/13/counterexample).

<a id="term-AnalyticsRule"></a>
#### `onvif:AnalyticsRule`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#AnalyticsRule` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the AnalyticsRule contract actually applies to the declared host. No same-named global XSD type is inferred. `LineCounting` denotes a rule type when used as such. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-AnalyticsRule](#term-AnalyticsRule) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/AnalyticsRule).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/AnalyticsRule","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:AnalyticsRule"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/14/counterexample).
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#Receiver` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the Receiver contract actually applies to the declared host. A configured URI does not establish reception. No implicit CreateReceiver or connection is permitted. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-Receiver](#term-Receiver) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/Receiver).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/Receiver","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/15/counterexample).

<a id="term-VideoSource"></a>
#### `onvif:VideoSource`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#VideoSource` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the VideoSource contract actually applies to the declared host. Source options do not imply a supported combination or sensor maximum. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-VideoSource](#term-VideoSource) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/VideoSource).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/VideoSource","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:VideoSource"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/16/counterexample).

<a id="term-AudioSource"></a>
#### `onvif:AudioSource`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#AudioSource` |
| Kind and representation | class; class IRI |
| Host | Native source resource |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the AudioSource contract actually applies to the declared host. Does not imply audio in every media profile, an output decoder, or backchannel. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-AudioSource](#term-AudioSource) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/AudioSource).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/AudioSource","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:AudioSource"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/17/counterexample).

<a id="term-DigitalInput"></a>
#### `onvif:DigitalInput`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#DigitalInput` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the DigitalInput contract actually applies to the declared host. Missing state is unknown. A Boolean `false` is a known value only with valid evidence. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-DigitalInput](#term-DigitalInput) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/DigitalInput).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/DigitalInput","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:DigitalInput"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/18/counterexample).

<a id="term-RelayOutput"></a>
#### `onvif:RelayOutput`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#RelayOutput` |
| Kind and representation | class; class IRI |
| Host | Native I/O resource |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the RelayOutput contract actually applies to the declared host. No actuation default. A successful command is not proof of external electrical or mechanical effect. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-RelayOutput](#term-RelayOutput) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/RelayOutput).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/RelayOutput","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:RelayOutput"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/19/counterexample).

<a id="term-SerialPort"></a>
#### `onvif:SerialPort`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#SerialPort` |
| Kind and representation | class; class IRI |
...
| Cardinality | Zero or one member of the @type IRI set; duplicate class membership has no added meaning. |
| Requiredness | Use only when the SerialPort contract actually applies to the declared host. No implicit byte transfer, guessed baud rate, or access to an OS serial device. |
| Omission and default | Omission asserts no class. No endpoint, resource, physical behavior, trust or conformance is created. |
...
| Security and privacy | Publish only the minimum authorized public metadata. Credential values, private device/driver selectors, process handles and routing grants MUST NOT appear. An annotation never authenticates its publisher. |
| Sources | [#vocabulary-classes](#vocabulary-classes); [#term-SerialPort](#term-SerialPort) |

Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/SerialPort).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/SerialPort","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "onvif:SerialPort"
    // ...
}
```

Invalid use at this host: @type requires an IRI or IRI array, not an XML QName object. XML QName identity and RDF class identity are distinct. [Rejected annotation](terms.json#/terms/20/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/operation).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/operation","retain":["/@context","/actions/getProfiles/forms/0/href","/actions/getProfiles/forms/0/op","/actions/getProfiles/forms/0/onvif:operation"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "actions": {
        "getProfiles": {
            "forms": [
                {
                    "href": "https://camera.example.invalid/media2",
                    "op": "invokeaction",
                    "onvif:operation": {
                        "bindingQName": {
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the operation record/value contract. [Rejected annotation](terms.json#/terms/21/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/source).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/source","retain":["/@context","/actions/getProfiles/onvif:source"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the source record/value contract. [Rejected annotation](terms.json#/terms/22/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/sourcePins).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/sourcePins","retain":["/@context","/@type","/onvif:sourcePins"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the sourcePins record/value contract. [Rejected annotation](terms.json#/terms/23/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/profileClaims).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/profileClaims","retain":["/@context","/onvif:profileClaims"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the profileClaims record/value contract. [Rejected annotation](terms.json#/terms/24/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/profileEdition).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/profileEdition","retain":["/@context","/@type","/onvif:profileEdition"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the profileEdition record/value contract. [Rejected annotation](terms.json#/terms/25/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/observedServices).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/observedServices","retain":["/@context","/onvif:observedServices"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the observedServices record/value contract. [Rejected annotation](terms.json#/terms/26/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/capabilityEvidence).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/capabilityEvidence","retain":["/@context","/onvif:capabilityEvidence"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the capabilityEvidence record/value contract. [Rejected annotation](terms.json#/terms/27/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/conformanceEvidence).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/conformanceEvidence","retain":["/@context","/onvif:conformanceEvidence"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the conformanceEvidence record/value contract. [Rejected annotation](terms.json#/terms/28/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/resourceIdentity).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/resourceIdentity","retain":["/@context","/onvif:resourceIdentity"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the resourceIdentity record/value contract. [Rejected annotation](terms.json#/terms/29/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/nativeIdentity).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/nativeIdentity","retain":["/@context","/@type","/onvif:nativeIdentity"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the nativeIdentity record/value contract. [Rejected annotation](terms.json#/terms/30/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/requirements).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/requirements","retain":["/@context","/@type","/onvif:requirements"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the requirements record/value contract. [Rejected annotation](terms.json#/terms/31/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/obligationGroups).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/obligationGroups","retain":["/@context","/@type","/onvif:obligationGroups"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the obligationGroups record/value contract. [Rejected annotation](terms.json#/terms/32/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/safeRead).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/safeRead","retain":["/@context","/actions/getProfiles/onvif:safeRead"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the safeRead record/value contract. [Rejected annotation](terms.json#/terms/33/counterexample).
...
<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlQName","retain":["/@context","/properties/value/onvif:xmlQName"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the xmlQName record/value contract. [Rejected annotation](terms.json#/terms/34/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/xmlTypeQName).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlTypeQName","retain":["/@context","/properties/value/onvif:xmlTypeQName"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the xmlTypeQName record/value contract. [Rejected annotation](terms.json#/terms/35/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/xmlFacets).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlFacets","retain":["/@context","/properties/value/onvif:xmlFacets"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the xmlFacets record/value contract. [Rejected annotation](terms.json#/terms/36/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/xmlWildcard).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlWildcard","retain":["/@context","/properties/value/onvif:xmlWildcard"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the xmlWildcard record/value contract. [Rejected annotation](terms.json#/terms/37/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/nativeConstraints).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/nativeConstraints","retain":["/@context","/properties/value/onvif:nativeConstraints"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the nativeConstraints record/value contract. [Rejected annotation](terms.json#/terms/38/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/diagnostics).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/diagnostics","retain":["/@context","/onvif:diagnostics"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the diagnostics record/value contract. [Rejected annotation](terms.json#/terms/39/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/profileAssessments).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/profileAssessments","retain":["/@context","/onvif:profileAssessments"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the profileAssessments record/value contract. [Rejected annotation](terms.json#/terms/40/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/inputBindings).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/inputBindings","retain":["/@context","/@type","/onvif:inputBindings"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the inputBindings record/value contract. [Rejected annotation](terms.json#/terms/41/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/readOutcomes).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/readOutcomes","retain":["/@context","/onvif:readOutcomes"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the readOutcomes record/value contract. [Rejected annotation](terms.json#/terms/42/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/subscription).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/subscription","retain":["/@context","/events/notifications/forms/0/href","/events/notifications/forms/0/op","/events/notifications/forms/0/onvif:subscription"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "events": {
        "notifications": {
            "forms": [
                {
                    "href": "https://camera.example.invalid/events",
                    "op": [
                        "subscribeevent",
                        "unsubscribeevent"
                    ],
...
Invalid use at this host: This is not the declared JSON literal representation and does not satisfy the subscription record/value contract. [Rejected annotation](terms.json#/terms/43/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/sourceDataSchema).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/sourceDataSchema","retain":["/@context","/properties/value/onvif:sourceDataSchema"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared IRI representation and does not satisfy the sourceDataSchema record/value contract. [Rejected annotation](terms.json#/terms/44/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/clientRequirements).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/clientRequirements","retain":["/@context","/@type","/onvif:clientRequirements"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared IRI representation and does not satisfy the clientRequirements record/value contract. [Rejected annotation](terms.json#/terms/45/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/serviceModels).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/serviceModels","retain":["/@context","/onvif:serviceModels"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared IRI representation and does not satisfy the serviceModels record/value contract. [Rejected annotation](terms.json#/terms/46/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/profileModels).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/profileModels","retain":["/@context","/onvif:profileModels"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared IRI representation and does not satisfy the profileModels record/value contract. [Rejected annotation](terms.json#/terms/47/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/modelClass).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/modelClass","retain":["/@context","/@type","/onvif:modelClass"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
    ...
    "@type": "tm:ThingModel",
    "onvif:modelClass": "https://example.org/wot/onvif#Semantic"
    // ...
}
```

Invalid use at this host: This is not the declared IRI representation and does not satisfy the modelClass record/value contract. [Rejected annotation](terms.json#/terms/48/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/binding).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/binding","retain":["/@context","/actions/getProfiles/forms/0/href","/actions/getProfiles/forms/0/op","/actions/getProfiles/forms/0/onvif:binding"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the binding record/value contract. [Rejected annotation](terms.json#/terms/49/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/soapAction).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/soapAction","retain":["/@context","/actions/getProfiles/forms/0/href","/actions/getProfiles/forms/0/op","/actions/getProfiles/forms/0/onvif:soapAction"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the soapAction record/value contract. [Rejected annotation](terms.json#/terms/50/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/registryDigest).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/registryDigest","retain":["/@context","/onvif:registryDigest"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the registryDigest record/value contract. [Rejected annotation](terms.json#/terms/51/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/requirementsDigest).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/requirementsDigest","retain":["/@context","/@type","/onvif:requirementsDigest"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the requirementsDigest record/value contract. [Rejected annotation](terms.json#/terms/52/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/projectionDigest).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/projectionDigest","retain":["/@context","/onvif:projectionDigest"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the projectionDigest record/value contract. [Rejected annotation](terms.json#/terms/53/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/canonicalType).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/canonicalType","retain":["/@context","/properties/value/onvif:canonicalType"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the canonicalType record/value contract. [Rejected annotation](terms.json#/terms/54/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/xmlKind).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlKind","retain":["/@context","/properties/value/onvif:xmlKind"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the xmlKind record/value contract. [Rejected annotation](terms.json#/terms/55/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/xmlDefault).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlDefault","retain":["/@context","/properties/value/onvif:xmlDefault"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the xmlDefault record/value contract. [Rejected annotation](terms.json#/terms/56/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/xmlFixed).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/xmlFixed","retain":["/@context","/properties/value/onvif:xmlFixed"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the xmlFixed record/value contract. [Rejected annotation](terms.json#/terms/57/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/validation).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/validation","retain":["/@context","/properties/value/onvif:validation"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the validation record/value contract. [Rejected annotation](terms.json#/terms/58/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/unsupported).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/unsupported","retain":["/@context","/properties/value/onvif:unsupported"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the unsupported record/value contract. [Rejected annotation](terms.json#/terms/59/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/mappingSupport).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/mappingSupport","retain":["/@context","/actions/getProfiles/onvif:mappingSupport"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the mappingSupport record/value contract. [Rejected annotation](terms.json#/terms/60/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/runtimeStatus).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/runtimeStatus","retain":["/@context","/actions/getProfiles/onvif:runtimeStatus"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the runtimeStatus record/value contract. [Rejected annotation](terms.json#/terms/61/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/sourceGroup).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/sourceGroup","retain":["/@context","/@type","/onvif:sourceGroup"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the sourceGroup record/value contract. [Rejected annotation](terms.json#/terms/62/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/serviceNamespace).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/serviceNamespace","retain":["/@context","/@type","/onvif:serviceNamespace"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the serviceNamespace record/value contract. [Rejected annotation](terms.json#/terms/63/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/resourceKind).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/resourceKind","retain":["/@context","/@type","/onvif:resourceKind"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the resourceKind record/value contract. [Rejected annotation](terms.json#/terms/64/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/authorship).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/authorship","retain":["/@context","/@type","/onvif:authorship"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the authorship record/value contract. [Rejected annotation](terms.json#/terms/65/counterexample).
...
Thing Model excerpt; `// ...` marks omitted members. [Complete Thing Model example](examples/vocabulary-examples.json#/requirementSemantics).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/requirementSemantics","retain":["/@context","/@type","/onvif:requirementSemantics"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the requirementSemantics record/value contract. [Rejected annotation](terms.json#/terms/66/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/claimSemantics).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/claimSemantics","retain":["/@context","/onvif:claimSemantics"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: This is not the declared literal representation and does not satisfy the claimSemantics record/value contract. [Rejected annotation](terms.json#/terms/67/counterexample).

<a id="term-Semantic"></a>
#### `onvif:Semantic`
...
| Field | Normative contract |
| --- | --- |
| IRI | `https://example.org/wot/onvif#Semantic` |
| Kind and representation | class; class IRI |
...
| Requiredness | REQUIRED on concrete semantic-fragment adapter TDs in this edition; forbidden as a substitute for evidence of native Device behavior. |
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/Semantic).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/Semantic","retain":["/@context","/@type"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: A non-native fragment declaration cannot simultaneously manufacture native-device behavior. [Rejected annotation](terms.json#/terms/68/counterexample).
...
Thing Description excerpt; `// ...` marks omitted members. [Complete Thing Description example](examples/vocabulary-examples.json#/projection).

<!-- td-excerpt: {"source":"examples/vocabulary-examples.json#/projection","retain":["/@context","/onvif:projection"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",
...
Invalid use at this host: Two camera fragments do not provide every applicable Profile T/M requirement or the required exact full-profile evidence. [Rejected annotation](terms.json#/terms/69/counterexample).
...
The claim categories `nativeMapping` and `wotSemanticProfile` are values in the declared opaque `onvif:projection` record, not classes or independent vocabulary terms. The additive `onvif:Semantic` class and `onvif:projection` property are inventoried with context, RDF, SHACL, JSON Schema, producer/consumer/default rules and four-space examples. Native-specific `NativeContract` typing and native binding selectors MUST NOT be attached to a non-ONVIF interface.
...
| --- | --- |
| `terms.json` | Sole authored 70-entry term inventory (68 original contracts under their current names plus Semantic and projection), enriched with Chapter 4's full contract metadata |
...
| Vocabulary entries | 70; 68 original contracts plus 2 declared model terms; class IRIs use the current suffix-free names |
...
| MODELTERMS | Semantic and projection are the only additive model terms, with complete inventory/context/RDF/SHACL/JSON record contracts; category/status values do not create undeclared terms |
...
| --- | --- |
| `cameras/t-m-camera.td.json` | T+M native camera, Device/Media2/Event contracts, partial observed composite, explicit media-profile tokens; excerpt below |
...
| `vocabulary-examples.json` | Complete illustrative TD/TM objects for every vocabulary term, addressed by term-name JSON Pointer; source for the Chapter 4 excerpts |
...
### E.2 Hypothetical T+M camera TD excerpt

The excerpt is projected from the [complete T+M camera TD](examples/cameras/t-m-camera.td.json), JSON Pointer `#`. Only the class and a selected native Action Form are shown; `// ...` marks omitted members. The native operation IDs and schema fragments come from the frozen catalog. The complete example advertises hypothetical T/M claims but deliberately exposes a partial observed interface; its type link identifies the companion observed composite. It is not a complete profile-conformance example.
...
<!-- td-excerpt: {"source":"examples/cameras/t-m-camera.td.json#","retain":["/@context","/@type","/actions/Media2Binding_GetProfiles_95aacf693b24/forms"]} -->
```jsonc
{
    "@context": [
        "https://www.w3.org/2022/wot/td/v1.1",

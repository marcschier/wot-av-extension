...
export function elementSchema(element: ElementDescriptor, registry: XmlRegistry, graph?: SchemaGraph, shared = false): Schema {
    ...
    return { ...(schemas.length === 1 ? schemas[0] : { oneOf: schemas }), ...(shared ? {} : { "onvif:xmlQName": element.name }),
        ...
        ...(element.unique === undefined ? {} : { "onvif:xmlUnique": element.unique,
            "onvif:nativeConstraints": ["Namespace-aware xs:unique selectors, field value spaces and per-element scope are checked by the codec, not JSON Schema uniqueItems."] }),

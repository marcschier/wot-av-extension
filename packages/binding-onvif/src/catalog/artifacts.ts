...
import { formSchema, requirementsSchema, snapshotSchema } from "./contracts.js";
import { assertModelImports, modelIndex } from "./model-resolver.js";
import { generateModels, generateVocabulary } from "./models.js";
import { loadRequirementIndex, type RequirementIndex } from "./requirements.js";
import { generatePayloadSchema, ONVIF_BASE } from "./schemas.js";
...
function ownedPath(root: string, resource: string): string {
    ...
    return join(root, "bindings", "onvif", ...parts);
}
...
export function compileArtifacts(root: string): {
    ...
    const models = generateModels(catalog, requirements), vocabulary = generateVocabulary(catalog);
    ...
    files.set("generated/mapping-reference.json", bytes({
        ...
        modelSemantics: "The operation library defines affordances once. Services/features/profile wrappers import them, and client obligations remain separate manifests.",
    ...
    const manifest = {
        ...
        artifacts: [...files].sort(([a], [b]) => a < b ? -1 : 1).map(([path, value]) => entry(path, value)),
...
export function stagePackagedArtifacts(root: string): void {
    const sourceBase = join(resolve(root), "bindings", "onvif");
    const destination = join(__dirname, "..", "catalog-data");
    const manifestBytes = readFileSync(join(sourceBase, "generated", "manifest.json"));
    ...
    for (const [name, source] of Object.entries(PACKAGE_ASSETS)) content.set(name, readFileSync(join(sourceBase, ...source.split("/"))));
...
export function generateOnvif(root: string, check = false): GenerationResult {
    ...
    if (!check) {
        for (const item of obsolete) {
            const path = ownedPath(root, item.path);

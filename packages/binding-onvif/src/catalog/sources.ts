...
export function loadLockedSources(options: LoadSourcesOptions): LockedClosure {
    ...
    const loaded = readLock(options.lockPath ?? join(root, "bindings", "onvif", "sources.lock.json"));
    const { lock } = loaded;
    ...
    const visit = (id: string): void => {
        ...
        const base = source.storage === "vendored" ? join(root, "third_party", "onvif") : options.cache;

...
import {
    actionTemplate, clientManifestId, context, DEVICE_MODEL, model, OPERATION_MODEL,
...
function composite(
    ...
    return { ...model(`${ONVIF_BASE}/models/composites/Observed-${hash}.tm.json`, "Observed native ONVIF interface", base),
        actions: imports, "onvif:registryDigest": catalog.registryDigest, "onvif:requirementsDigest": index.digest,
        description: "One immediate base and imported observed native affordances. This is not a complete profile-wrapper instance or a certification statement." };
}
...
export function project(snapshot: DeviceSnapshot, catalog: ProjectionCatalog, policy: ProjectionPolicy): ProjectionSet {
    ...
    if (snapshot.services.length || snapshot.resources.length || claims.some((claim) => claim.role === "device")) {
        ...
        const id = String(composed.id); models.set(id, composed);
    ...
    for (const [id, resource] of [...resourceMap].sort(([a], [b]) => a < b ? -1 : 1)) {
        ...
        const modelId = String(composed.id); models.set(modelId, composed);

...
schemas = [
    json.loads((base / "schemas" / name).read_text(encoding="utf-8"))
    for name in (
...
td_schema = json.loads(
    (root / "third_party" / "wot" / "td-schema.json").read_text(encoding="utf-8")
)
contexts = {
    "https://www.w3.org/2022/wot/td/v1.1": json.loads(
        (root / "third_party" / "wot" / "td-context.jsonld").read_text(encoding="utf-8")
    ),
    "https://example.org/wot/onvif/context/v0.1": json.loads(
        (base / "vocabulary" / "context.jsonld").read_text(encoding="utf-8")
    ),
}
...
for name in ("requirements-st.json", "requirements-gm.json", "requirements-acd.json"):
    value = json.loads((base / "catalog" / name).read_text(encoding="utf-8"))

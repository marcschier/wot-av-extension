...
const { generatePayloadSchema, simpleSchema } = require("../../dist/catalog/schemas");
const { safeReadAliases } = require("../../dist/catalog/mappings");
const { resolveType } = require("../../dist/xml/descriptors");

const root = resolve(__dirname, "..", "..", "..", "..");
const catalog = compileCatalog({ root });
const native = require("./fixtures/native-responses.json");
...
test("catalog: every locked native binding operation is independently reconciled, without wsdl:service", () => {
    const lock = JSON.parse(readFileSync(join(root, "bindings", "onvif", "sources.lock.json"), "utf8"));
    ...
    for (const id of selected) {
        ...
        const bytes = readFileSync(join(root, "third_party", "onvif", ...source.relativePath.split("/")));

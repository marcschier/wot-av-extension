...
param(
    ...
    [string]$BuildRoot = (Join-Path $PSScriptRoot '..\build-p0'),
...
if (-not $BuildRoot.StartsWith("$project\", [StringComparison]::OrdinalIgnoreCase)) {
    throw 'BuildRoot must be below the owned native\onvif-media directory.'
}
...
if ($Test) {
    & python -B (Join-Path $project 'test\cohort_gate.py') --build-root $BuildRoot --sdk-root $SdkRoot --ctest $ctest
    if ($LASTEXITCODE -ne 0) {

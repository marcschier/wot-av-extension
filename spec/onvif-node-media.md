...
[native media qualification](onvif-media-qualification.md) for the separate
...
& $node .\node_modules\typescript\bin\tsc -p .\packages\binding-onvif\test\media\tsconfig.json
& $node .\node_modules\typescript\bin\tsc -p .\packages\binding-onvif\test\media\tsconfig.unit.json
$tests = @(Get-ChildItem .\packages\binding-onvif\test\media -Filter "*.test.cjs" -File | ForEach-Object FullName)
...
$env:ONVIF_MEDIA_WORKER = (Resolve-Path .\native\onvif-media\build-p0\bin\onvif_media_worker.exe).Path
...
& $node --test --test-timeout=50000 .\packages\binding-onvif\test\media\native.integration.cjs
```

...
redistributable product. [Qualification](onvif-media-qualification.md) separates
...
locked by `native\onvif-media\sources.lock.json`. The build verifies all 28
...
```powershell
python -B .\native\onvif-media\test\cohort_gate.py `
  --build-root .\native\onvif-media\build-p0 `

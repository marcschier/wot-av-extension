...
The implementation is isolated in `packages\binding-onvif\src\discovery`.
...
```powershell
node node_modules\typescript\bin\tsc -p packages\binding-onvif\test\discovery\tsconfig.json
$tests = Get-ChildItem packages\binding-onvif\test\discovery -File -Filter '*.test.cjs'
...
The protocol source lock is `bindings\onvif\sources.lock.json`, nativeDiscovery

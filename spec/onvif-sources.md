...
[`sources.lock.json`](../bindings/onvif/sources.lock.json) pins ONVIF release
...
The [profile-edition catalog](../bindings/onvif/catalog/profile-editions.json)
...
`third_party\onvif\` or an explicitly supplied cache. Filesystem callers must split
...
local_file = root.joinpath("third_party", "onvif", *dependency["relativePath"].split("/"))
```
...
python -B tools\fetch_onvif_sources.py --check --scope vendored
```
...
python -B tools\fetch_onvif_sources.py --fetch --cache $cache --scope xml
python -B tools\fetch_onvif_sources.py --check --cache $cache --scope xml
```
...
python -B tools\fetch_onvif_sources.py --export $export --scope vendored
python -B tools\fetch_onvif_sources.py --check --lock (Join-Path $export 'sources.lock.json') --root $export --cache $export --scope all
```
...
The [source policy](../bindings/onvif/catalog/source-policy.json) records the
...
The [editorial register](../bindings/onvif/catalog/editorial-decisions.json)

...
The current canonical manifest contains **305 artifacts including itself, of
which 292 are model documents**; 305 is not a model-document count. The package
...
`tools\generate_onvif.py` writes only `onvif-release-manifest.json`; it hashes
...
python -B tools\generate_onvif.py
python -B tools\generate_onvif.py --check
python -B tools\validate_onvif.py --suite unit --suite catalog
...
```powershell
python -B tools\validate_onvif.py --allow-loopback --suite events --suite integration
```
...
It configures/builds in the separately owned `native\onvif-media\build-package-ci`,

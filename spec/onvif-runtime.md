...
Copy `examples\onvif-bridge-fixture.json` or
`examples\onvif-bridge-directory.json` into a private operator-owned directory
...
node .\packages\binding-onvif\dist\cli\main.js inspect --config $config
node .\packages\binding-onvif\dist\cli\main.js export --config 'C:\onvif-lab\fixture-export.json'
node .\packages\binding-onvif\dist\cli\main.js run --config $config --once
node .\packages\binding-onvif\dist\cli\main.js run --config $config
```
...
The current [logical endpoint gate](../packages/binding-onvif/src/security/logical-endpoint.ts)
separates routing identity from the physical Form target.
[`bridge.test.cjs`](../packages/binding-onvif/test/integration/bridge.test.cjs)
...
[native worker qualification](onvif-media-qualification.md) now have their own
...
See the [support matrix](onvif-conformance.md) and

...
$lock = Get-Content -LiteralPath (Join-Path $root 'native\onvif-media\dependencies.lock.json') -Raw | ConvertFrom-Json
...
Push-Location $root
try {
    & (Join-Path $root 'native\onvif-media\tools\build.ps1') -SdkRoot $SdkRoot -SourceRoot $SourceRoot `
        -BuildRoot $BuildRoot -NodeExecutable $NodeExecutable
    & python -B (Join-Path $root 'native\onvif-media\test\cohort_gate.py') `

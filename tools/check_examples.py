...
def run_authored_oracle(checks, path):
    ...
    def document(name):
        if name not in cached:
            cached[name] = read(relative_file("examples/" + name))
        return cached[name]
...
def run(checks):
            ...
            for offer in document.get("av:offer", []):
                ...
                for mode in offer["av:mode"]:
                    ...
                    if form_id in expectations["unsupportedNativeAccess"]:
                        negative("Native template access fails explicitly: " + form_id, resolve,
                                 match=r"^Unsupported URI template:",
                                 category="documented-native-limit")
    ...
    two_inputs = documents["examples/two-input-need.jsonld"]["av:input"]
    ...
    result_need = documents["examples/result-need.jsonld"]
    ...
    source = documents["examples/source.td.json"]
    live = documents["examples/live.td.json"]
    ...
    requirement = documents["examples/live-need.jsonld"]["av:input"][0]
    ...
    adapter = read(relative_file("examples/onvif-adapter.json"))
    ...
    for name in ("examples/pins.jsonld", "examples/assignment.jsonld", "examples/controller.td.json"):
        check("Retired machinery exists only in the historical snapshot: " + name,
              not relative_file(name).exists())
    check("No active profile document family remains", not list((relative_file("examples") / "profiles").glob("*.json")))
    return {

...
def main():
    ...
    commands = [
        [sys.executable, "-B", str(ROOT / "tools" / "fetch_onvif_sources.py"), "--check", "--scope", "vendored"],
        [sys.executable, "-B", "-m", "unittest", "discover", "-s", str(ROOT / "tests" / "onvif_sources"), "-p", "test_*.py"],
        [node, str(ROOT / "node_modules" / "typescript" / "bin" / "tsc"), "-p", str(ROOT / "packages" / "binding-onvif" / "tsconfig.json")],
        [node, str(ROOT / "tools" / "generate-onvif.cjs"), "--check"],
        [node, str(ROOT / "tools" / "stage-onvif-package.cjs")],
        [sys.executable, "-B", str(ROOT / "tools" / "generate_onvif.py"), "--check"]
    ]
    if args.suite:
        commands.append([node, str(ROOT / "packages" / "binding-onvif" / "test" / "run.cjs"), *args.suite])

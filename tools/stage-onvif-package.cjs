...
const source = join(root, "bindings", "onvif");
const dist = join(root, "packages", "binding-onvif", "dist");
...
const generatedPath = join(source, "generated", "manifest.json");
...
for (const entry of generated.artifacts) {
    ...
    copy(`catalog-data/${entry.path}`, join(source, ...entry.path.split("/")), entry);
}
...
for (const entry of vendored) {
    ...
    copy(`source-documents/${entry.relativePath}`, join(root, "third_party", "onvif", ...entry.relativePath.split("/")), entry);
}
copy("source-documents/sources.lock.json", lockPath);
copy("source-documents/source-policy.json", join(source, "catalog", "source-policy.json"));

const inputs = [];
const sourceRoot = join(root, "packages", "binding-onvif", "src");
...
audit(dist);
for (const name of ["package.json", "package-lock.json", "packages/binding-onvif/package.json",
    "packages/binding-onvif/tsconfig.json", "packages/binding-onvif/NOTICE.md", "tools/stage-onvif-package.cjs"]) {
